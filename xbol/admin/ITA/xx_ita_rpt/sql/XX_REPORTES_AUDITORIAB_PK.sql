...
REM +======================================================================+
REM |  FILENAME                                                            |
REM |    XX_REPORTES_AUDITORIAB_PK.sql                                     |
REM |                                                                      |
REM |  DESCRIPTION                                                         |
REM |    Crea el cuerpo del paquete xx_reportes_auditoria_pkg              |
REM |                                                                      |
REM |  LANGUAGE                                                            |
REM |    PL/SQL                                                            |
REM |                                                                      |
...
REM |                                                                      |
REM |  HISTORY                                                             |
REM |    15-SEP-17 - DVartabedian - DSP - Created                          |
REM |                                                                      |
REM | NOTES                                                                |
REM |                                                                      |
REM +======================================================================+

SPOOL XX_REPORTES_AUDITORIAB_PK.log

prompt =====================================================================
prompt script XX_REPORTES_AUDITORIAB_PK.sql
prompt =====================================================================

prompt creando el cuerpo del paquete xx_reportes_auditoria_pkg

create or replace package body xx_reportes_auditoria_pkg as
/* $Header: XX_REPORTES_AUDITORIAB_PK.sql 1.0 2017-09-15 14:30:00 daniel.vartabedian@despegar.com $ */
/*+======================================================================+*/
/*|           Copyright (c) 2017 Oracle Argentina, Buenos Aires          |*/
/*|                         All rights reserved.                         |*/
/*+======================================================================+*/
/*|  FILENAME                                                            |*/
/*|    XX_REPORTES_AUDITORIAB_PK.sql                                     |*/
/*|                                                                      |*/
/*|  DESCRIPTION                                                         |*/
/*|    Crea el cuerpo del paquete xx_reportes_auditoria_pkg              |*/
/*|                                                                      |*/
/*|  LANGUAGE                                                            |*/
/*|    PL/SQL                                                            |*/
/*|                                                                      |*/
...
/*|                                                                      |*/
/*|  HISTORY                                                             |*/
/*|    15-SEP-17 - DVartabedian - DSP - Created                          |*/
/*|                                                                      |*/
/*| NOTES                                                                |*/
/*|                                                                      |*/
/*+======================================================================+*/
...
EXCEPTION
  WHEN NO_DATA_FOUND THEN
       show_debug('No se encontró el usuario '||NVL(p_user_id,Fnd_Global.user_id));
       RETURN( TO_CHAR(NULL) );
  WHEN OTHERS THEN
...
BEGIN
  --
  -- Si el l_to está vacío, pero hay datos en el l_cc, entonces mando el l_cc a l_to, y dejo el l_cc vacío.
...
EXCEPTION
  WHEN NO_DATA_FOUND THEN
       show_debug('No se encontró la lista de distribución: '||p_distribution_list);
       x_to := TO_CHAR(NULL);
       x_cc := TO_CHAR(NULL);
  WHEN OTHERS THEN
       show_debug('Error al buscar la lista de distribución: '||p_distribution_list||'; '||SQLERRM);
       RAISE_APPLICATION_ERROR(-20001,'Error al buscar la lista de distribución: '||p_distribution_list||'; '||SQLERRM);
END get_distribution_list;

-- -----------------------------------------------------------------------------
-- Name   : with_text_attach
-- Purpose: Envía un mail con un archivo de texto attachado.
-- Params : p_to      : puede ser en formato 'Daniel Vartabedian <daniel.vartabedian@ua.com.ar>'
--          p_cc      : ídem.
--          p_subject : asunto; se puede formatear con códigos html; por ej.: '<h1>Esto es el body</h1>'
--          p_body    : cuerpo del mail: ídem.
...
BEGIN
   ...
   -- El primer llamado al attach_text es para el body (se puede formatear con códigos HTML).
...
--             write_raw(conn => l_conn, message => l_message);           -- para archivos de text no funciona.
         ...
         EXCEPTION
            WHEN OTHERS THEN  -- siempre sale por aquí, cuando deja de encontrar datos.
                 NULL;
         END;
         --
...
-- Purpose: Envía un mail con un archivo binario (por ejemplo, un .pdf) attachado.
-- Params : p_to      : puede ser en formato 'Daniel Vartabedian <daniel.vartabedian@ua.com.ar>'
--          p_cc      : ídem.
--          p_subject : asunto; se puede formatear con códigos html; por ej.: '<h1>Esto es el body</h1>'
--          p_body    : cuerpo del mail: ídem.
...
BEGIN
   ...
   l_conn := begin_mail( sender         => c_from_name,
                         ...
                         mime_type       => c_multipart_mime_type );
   show_debug ('  Hizo el begin_mail');
   -- El primer llamado al attach_text es para el subject y el body (se puede formatear con códigos HTML).
   attach_mb_text( conn       => l_conn,
...
-- Purpose: Envía un mail con un archivo de texto attachado con el contenido del body.
-- Params : p_to      : puede ser en formato 'Daniel Vartabedian <daniel.vartabedian@ua.com.ar>'
--          p_cc      : ídem.
--          p_subject : asunto; se puede formatear con códigos html; por ej.: '<h1>Esto es el body</h1>'
--          p_body    : cuerpo del mail: ídem.
...
-- -----------------------------------------------------------------------------------------------------------
-- Name   : send_mail
-- Purpose: Envía un mail con/sin un archivo de attachado.
...
--          p_location: ubicación del archivo que se va a attachar.
...
--  IF NVL(p_body_attach,'N') = 'Y' OR p_filename IS NULL THEN
   IF NVL (p_body_attach, 'N') = 'Y' THEN
      -- Envía el mail con el body attachado en un archivo de texto (si p_filename is not null)
      -- o sinó, manda un mail común (con subject y body).
   ...
   ELSE
      ...
      ELSE
         IF l_extension IN ('.PDF') THEN
            -- Envía mail con archivo binario attachado (debe estar en el fnd_profile.value('ECX_UTL_LOG_DIR_OBJ')).
            with_binary_attach (p_to            => p_to,
                                ...
                                p_location      => p_location);
         ELSE
            -- Envía un mail con un archivo de texto attachado.
            with_text_attach (p_to            => p_to,
                              p_cc            => p_cc,
                              p_subject       => p_subject,
                              p_body          => p_body,
...
--            
         SELECT user_name, responsibility_name, full_name, country, end_date_direct, end_date_user, end_date_resp, supervisor, super_supervisor, request_group_name, menu_name
               ,email_address, creation_date, created_by, last_update_date, updated_by, last_logon_date  
           FROM (
         SELECT fu.user_name
               ,resp.responsibility_name
               ,he.full_name
               ,pa.d_set_of_books_id country 
               ,pa.d_supervisor_id  supervisor
               ,furg.end_date  end_date_direct
               ,fu.end_date    end_date_user
               ,resp.end_date  end_date_resp
               ,(SELECT d_supervisor_id FROM per_assignments_v7 WHERE person_id = pa.supervisor_id AND TRUNC(SYSDATE) BETWEEN effective_start_date AND effective_end_date) super_supervisor
               ,fu.email_address
              ,(SELECT request_group_name FROM fnd_request_groups rg WHERE rg.request_group_id = resp.request_group_id) request_group_name
               ,(SELECT menu_name FROM fnd_menus_vl fm WHERE fm.menu_id = resp.menu_id) menu_name
               ,furg.creation_date, (SELECT U.user_name FROM fnd_user U WHERE U.user_id = furg.created_by) created_by
               ,furg.last_update_date, (SELECT U.user_name FROM fnd_user U WHERE U.user_id = furg.last_updated_by) updated_by      
               ,fu.last_logon_date
               ,fu.user_id, resp.responsibility_id, pa.supervisor_id, pa.person_id, resp.application_id, 'D' direct
           FROM per_assignments_v7          pa
               ,hr_employees                he
               ,fnd_responsibility_vl       resp
               ,fnd_user_resp_groups_direct furg
               ,apps.fnd_user               fu
          WHERE 1=1
            AND (p_user_name IS NULL OR (p_user_name IS NOT NULL AND UPPER(fu.user_name) LIKE UPPER(TRIM(p_user_name))||'%'))
            AND (p_resp_name IS NULL OR (p_resp_name IS NOT NULL AND UPPER(resp.responsibility_name) LIKE UPPER(TRIM(p_resp_name))||'%'))
            AND furg.user_id = fu.user_id
            AND resp.responsibility_id = furg.responsibility_id
            AND resp.application_id = furg.responsibility_application_id
            AND he.employee_id(+) = fu.employee_id
            AND pa.person_id(+) = fu.employee_id
            AND (   (NVL(UPPER(p_con_bajas),'N') = 'Y') 
                 OR (NVL(UPPER(p_con_bajas),'N') = 'N' AND              
                     NVL(furg.end_date,TO_DATE('31124712','ddmmyyyy')) >= TRUNC(SYSDATE) AND
                     NVL(resp.end_date,TO_DATE('31124712','ddmmyyyy')) >= TRUNC(SYSDATE) AND
                     NVL(fu.end_date,TO_DATE('31124712','ddmmyyyy')) >= TRUNC(SYSDATE)) )            
         UNION ALL
         SELECT fu.user_name
               ,resp.responsibility_name
               ,pp.full_name
               ...
               ,furg.last_update_date, (SELECT U.user_name FROM fnd_user U WHERE U.user_id = furg.last_updated_by) updated_by      
               ,fu.last_logon_date
               ,fu.user_id, resp.responsibility_id, pa.supervisor_id, pa.person_id, resp.application_id, 'I' direct
           FROM per_assignments_v7            pa
               ,per_people_v7                 pp
               ,fnd_responsibility_vl         resp
            ...
            AND (   (NVL(UPPER(p_con_bajas),'N') = 'Y') 
                 OR (NVL(UPPER(p_con_bajas),'N') = 'N' AND              
                     ...
                     NVL(fu.end_date,TO_DATE('31124712','ddmmyyyy')) > TRUNC(SYSDATE)) )
                      ...
                      WHEN p_user_name IS NOT NULL AND p_resp_name IS NULL THEN responsibility_name
                      WHEN p_user_name IS NULL AND p_resp_name IS NOT NULL THEN user_name
                      WHEN p_user_name IS NOT NULL AND p_resp_name IS NOT NULL THEN NULL
                      WHEN p_user_name IS NULL AND p_resp_name IS NULL THEN NULL
                    END;
BEGIN
  ...
  fnd_file.put_line(fnd_file.LOG, 'Abro UTL_FILE (-)');
  -- Titulos.
  fnd_file.put_line(fnd_file.LOG, 'Títulos (+)');
...
--  fnd_file.put_line(fnd_file.output, ' ');
  ...
  EXCEPTION
    WHEN OTHERS THEN
         RAISE_APPLICATION_ERROR(-20001,'Error: no se encontró el email del usuario actual; '||SQLERRM);
  END;
...
EXCEPTION
    WHEN OTHERS THEN
         DBMS_OUTPUT.PUT_LINE('ERROR General; '||SQLERRM); --> no me interesa hacer un RAISE porque el proceso ya finalizó, y no lo voy a parar por esto.
END reporte_usuarios_respo;

-- -----------------------------------------------------------------------------
-- Name   : reporte_menu_conc
-- Purpose: genera un reporte con el árbol de menú de una responsabilidad.
...
-- Notes  : este procedure es invocado por: XX Reporte de Menúes y Concurrentes
-- Return :
-- History: 12/04/2016  Created
-- -----------------------------------------------------------------------------
...
IS
  ...
  l_kreg                        NUMBER := 0;
  l_to                          VARCHAR2(50);
  -- Este cursor se utiliza cuando el parámetro p_con_detalle = 'N'.
  CURSOR ccur1 IS
         SELECT * FROM (
                  SELECT LEVEL, fme.entry_sequence, LPAD(' ',(LEVEL-1)*5)||fme.PROMPT PROMPT, fff.TYPE, fff.function_name, fff.user_function_name, fme.menu_id, fme.sub_menu_id
                    FROM fnd_form_functions_vl fff
                        ,fnd_menu_entries_vl   fme
                   WHERE 1=1
                     AND fff.function_id(+) = fme.function_id
                     AND fme.PROMPT IS NOT NULL
                     AND fme.grant_flag = 'Y'                       --> con esto elimino las funciones excluidas
                     ...
                     -- excluye los tipo funci�n.
                     AND NOT EXISTS ( SELECT 1 
                                        FROM fnd_resp_functions    rf 
                                            ,fnd_responsibility_vl fr
                                       WHERE rf.rule_type = 'F'
                                         AND rf.responsibility_id = fr.responsibility_id
                                         AND UPPER(fr.responsibility_name) = UPPER(LTRIM(RTRIM(p_resp_name)))
                                         AND rf.action_id = fff.function_id )
                  CONNECT BY PRIOR fme.sub_menu_id = fme.menu_id
                     AND fme.PROMPT IS NOT NULL
                     -- excluye los tipos men�.
                     AND (SELECT DECODE( COUNT(1), 0, 'OK', 'BAJAR' )  
                            FROM fnd_resp_functions    rf 
                                ,fnd_responsibility_vl fr
                           WHERE rf.rule_type = 'M'
                             AND rf.responsibility_id = fr.responsibility_id
                             AND UPPER(fr.responsibility_name) = UPPER(LTRIM(RTRIM(p_resp_name)))
                             AND rf.action_id IN (fme.menu_id,fme.sub_menu_id) ) = 'OK'
                   START WITH fme.menu_id = (SELECT menu_id FROM fnd_responsibility_vl WHERE UPPER(responsibility_name) = UPPER(LTRIM(RTRIM(p_resp_name))))
                     AND fme.PROMPT IS NOT NULL
                     AND fme.grant_flag = 'Y'              --> con esto elimino los submenues excluidos
                    ORDER SIBLINGS BY fme.entry_sequence
                ) x
          WHERE 1=1
            -- con esto filtro los submen�s sin hijos, que se generan al aplicar las exclusiones.
            AND (SELECT DECODE( COUNT(1), 0, 'OK', 'BAJAR' )  
                   FROM fnd_resp_functions    rf 
                       ,fnd_responsibility_vl fr
                  WHERE rf.rule_type = 'M'
                    AND rf.responsibility_id = fr.responsibility_id
                    AND UPPER(fr.responsibility_name) = UPPER(LTRIM(RTRIM(p_resp_name)))
                    AND rf.action_id IN (x.menu_id,x.sub_menu_id) ) = 'OK';
  --
  -- Este cursor se utiliza cuando el parámetro p_con_detalle = 'Y'.
  CURSOR ccur2 IS
...
BEGIN
  ...
  fnd_file.put_line(fnd_file.LOG, 'Abro UTL_FILE (-)');
  -- Titulos.
  fnd_file.put_line(fnd_file.LOG, 'Titulos (+)');
  UTL_FILE.put_line(l_fhandle, l_titulo);
  fnd_file.put_line(fnd_file.output, l_titulo);
  UTL_FILE.put_line(l_fhandle, ' ');
  fnd_file.put_line(fnd_file.output, ' ');
  ...
  -- Recorro el cursor principal.
  fnd_file.put_line(fnd_file.LOG, 'Recorre CURSOR principal (+)');
-- Elimino la entrada de p_con_detalle = 'Y', porque este cursor no est� funcionando correctamente. DV-12/09/2017.
...
--  END IF;
  ...
  fnd_file.put_line(fnd_file.LOG, 'Se seleccionaron '||l_kreg||' registros.');
  --
  -- Reporte del árbol de menú.
  IF NVL(p_con_conc,'N') <> 'N' THEN
     fnd_file.put_line(fnd_file.LOG,'Reporte del arbol de Menu (+)');
     fnd_file.put_line(fnd_file.output,' ');
     fnd_file.put_line(fnd_file.output,' ');
     fnd_file.put_line(fnd_file.output,' ');
     UTL_FILE.put_line(l_fhandle, ' ');
  ...
  EXCEPTION
    WHEN OTHERS THEN
         RAISE_APPLICATION_ERROR(-20001,'Error: no se encontro el email del usuario actual; '||SQLERRM);
  END;
  fnd_file.put_line(fnd_file.LOG,'Envia mail a '||NVL(l_to,'')||' (+)');
  send_mail( p_to          => l_to
            ,p_cc          => NULL
...
-- Purpose: busca en qué responsabilidades se encuentra determinado concurrente.
...
--                      
         SELECT concurrent_program_name
               ...
               ,request_group_name
               ,request_group_code
               ,NVL(fa.product_code,fa.application_short_name) appl
          ...
          WHERE 1=1
            AND (p_conc_name IS NULL OR (p_conc_name IS NOT NULL AND UPPER(fcp.user_concurrent_program_name) LIKE UPPER(TRIM(p_conc_name))||'%'))
            AND (p_resp_name IS NULL OR (p_resp_name IS NOT NULL AND UPPER(resp.responsibility_name) LIKE UPPER(p_resp_name)||'%'))            
         --  AND frgu.request_unit_type = 'P'
            ...
            AND resp.request_group_id(+) = frgu.request_group_id;
BEGIN
  fnd_file.put_line(fnd_file.LOG, l_procname||' (+)');
  fnd_file.put_line(fnd_file.LOG, 'Params: ');
