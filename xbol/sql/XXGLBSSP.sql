...
--  written consent. 
-- 
--  File Name: XXGLBSSP.sql
--  Attribute Label: XXGL
-- 
--  Description: XX GL Reporte Balance Sumas y Saldos con cuenta PUC
-- 
...
declare
  ...
  is
    select gl.name                                      libro
          ...
          ,sum(gb.begin_balance_dr-gb.begin_balance_cr) saldo_inicial
          ,sum(gb.period_net_dr)                        debitos
          ,sum(gb.period_net_cr)                        creditos
          ,sum((gb.begin_balance_dr+gb.period_net_dr)-gb.period_net_cr) saldo_final
    ...
    where  1=1
    and    gb.code_combination_id             = gcc_corp.code_combination_id
    ...
    order by gcc_corp.segment2
            ,gcc_corp.segment3;
  /*--Parametros--*/
  p_access_set_id          gl_ledgers.ledger_id%type                 := '&&1';
  p_ledger_name            gl_ledgers.name%type                      := '&&2';
  p_ledger_id              gl_ledgers.ledger_id%type                 := '&&3';
  p_chart_of_accounts_id   gl_access_sets.chart_of_accounts_id%type  := '&&4';
  p_ledger_currency        fnd_currencies.currency_code%type         := '&&5';
  p_period_name            gl_periods.period_name%type               := '&&6';
  p_cta_flex_value_set_id  fnd_flex_values_vl.flex_value_set_id%type := '&&7';
  p_scta_flex_value_set_id fnd_flex_values_vl.flex_value_set_id%type := '&&8';
  p_coa_mpping_id          gl_cons_flexfield_map.coa_mapping_id%type := '&&9';
  p_puc_flex_value_set_id  fnd_flex_values_vl.flex_value_set_id%type := '&&10';
  p_ext_debug              varchar2(3)                               := '&&11';
  p_char_delim             varchar2(3)                               := '&&12';
  /*--Variables--*/
  v_dummy            boolean;
...
begin
  ...
  print_trace(p_debug=>p_ext_debug,p_str=>'--     1-p_access_set_id..........: '||p_access_set_id||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--     2-p_ledger_name............: '||p_ledger_name||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--     3-p_ledger_id..............: '||p_ledger_id||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--     4-p_chart_of_accounts_id...: '||p_chart_of_accounts_id||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--     5-p_ledger_currency........: '||p_ledger_currency||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--     6-p_period_name............: '||p_period_name||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--     7-p_cta_flex_value_set_id..: '||p_cta_flex_value_set_id||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--     8-p_scta_flex_value_set_id.: '||p_scta_flex_value_set_id||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--     9-p_coa_mpping_id..........: '||p_coa_mpping_id||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--    10-p_puc_flex_value_set_id..: '||p_puc_flex_value_set_id||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--    11-p_ext_debug..............: '||p_ext_debug||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--    12-p_char_delim.............: '||p_char_delim||'.');
  print_trace(p_debug=>p_ext_debug,p_str=>'--  Parametros(-)');
  ...
  for c_data in c_ar_data_print(c_ledger_id              => p_ledger_id
                               ,c_period_name            => p_period_name
                               ,c_cta_flex_value_set_id  => p_cta_flex_value_set_id
                               ,c_scta_flex_value_set_id => p_scta_flex_value_set_id
                               ,c_coa_mpping_id          => p_coa_mpping_id
                               ,c_puc_flex_value_set_id  => p_puc_flex_value_set_id)
  loop
    print_output(p_debug=>p_ext_debug
                ,p_str=>c_data.libro
